...
angular.module('timbercrate').controller('contactCtrl', function ($scope, mainService) {
...
angular.module('timbercrate').controller('mainCtrl', function ($scope, mainService, $state) {
  ...
  mainService.getFeatured().then(function (response) {
...
angular.module('timbercrate').controller('productCtrl', function ($scope, mainService) {
...
!function () {
  ...
  angular.module("ngMask").directive("mask", ["$log", "$timeout", "MaskService", function (a, b, c) {
    return { restrict: "A", require: "ngModel", compile: function compile(d, e) {
        function f(a) {
          "number" == typeof a && (b.cancel(g), g = b(function () {
            var b = a + 1,
                c = d[0];if (c.setSelectionRange) c.focus(), c.setSelectionRange(a, b);else if (c.createTextRange) {
              var e = c.createTextRange();e.collapse(!0), e.moveEnd("character", b), e.moveStart("character", a), e.select();
            }
          }));
        }if (!e.mask || !e.ngModel) return void a.info("Mask and ng-model attributes are required!");var g,
            h,
            i = c.create();return { pre: function pre(a, b, c) {
            h = i.generateRegex({ mask: c.mask, repeat: c.repeat || c.maskRepeat, clean: "true" === (c.clean || c.maskClean), limit: "true" === (c.limit || c.maskLimit || "true"), restrict: c.restrict || c.maskRestrict || "select", validate: "true" === (c.validate || c.maskValidate || "true"), model: c.ngModel, value: c.ngValue });
          }, post: function post(c, d, e, g) {
            h.then(function () {
              function h(b) {
                var c = b;b = b || "";var d = i.getViewValue(b),
                    e = k.maskWithoutOptionals || "",
                    h = d.withDivisors(!0),
                    j = d.withoutDivisors(!0);try {
                  var l = i.getRegex(h.length - 1),
                      m = i.getRegex(e.length - 1),
                      n = l.test(h) || m.test(h),
                      o = b.length - h.length === 1,
                      p = e.length - h.length > 0;if ("accept" !== k.restrict) if ("select" !== k.restrict || n && !o) "reject" !== k.restrict || n || (d = i.removeWrongPositions(h), h = d.withDivisors(!0), j = d.withoutDivisors(!0));else {
                    var q = b[b.length - 1],
                        r = h[h.length - 1];q !== r && p && (h += q);var s = i.getFirstWrongPosition(h);angular.isDefined(s) && f(s);
                  ...
                  k = i.getOptions();g.$parsers.push(h), d.on("click input paste keyup", function () {
                j = b(function () {
    ...
    function d() {
      function d(a, b) {
        var c;try {
          var d = t[a],
              e = C[d],
              f = h(a);e ? c = "(" + e.source + ")" : (i(a) || (z.push(a), A[a] = d), c = "(\\" + d + ")");
        } catch (g) {
          throw g;
        }return (f || b) && (c += "?"), new RegExp(c);
      }function e(a, b) {
        var c, f;try {
          var g = d(a, b);c = g;var i = h(a),
              j = g.source;if (i && u > a + 1) {
            var k = e(a + 1, !0).elementOptionalRegex();j += k.source;
          }f = new RegExp(j);
        } catch (l) {
          throw l;
        }return { elementRegex: function elementRegex() {
            return c;
          }, elementOptionalRegex: function elementOptionalRegex() {
            return f;
          } };
      }function f(c) {
        var d = a.defer();s = c;try {
          var f = c.mask,
              g = c.repeat;if (!f) return;g && (f = Array(parseInt(g) + 1).join(f)), w = b.getOptionals(f).fromMaskWithoutOptionals(), s.maskWithoutOptionals = t = b.removeOptionals(f), u = t.length;for (var h, i = 0; u > i; i++) {
            var l = e(i),
                m = l.elementRegex(),
                n = l.elementOptionalRegex(),
                o = h ? h.source + n.source : n.source;o = new RegExp(o), h = h ? h.source + m.source : m.source, h = new RegExp(h), B.push(o);
        ...
        function a(a, b) {
          ...
          for (var e = b[d], f = 1; f <= c.length; f++) {
            var g = c[f - 1];if (g >= e) break;x[e] = x[e] ? x[e].concat(e - f) : [e - f], A[e - f] = A[e];
          }
        }
      }function k(a) {
        a = a.toString();try {
          if (z.length > 0 && a) {
            for (var b = Object.keys(A), d = [], e = b.length - 1; e >= 0; e--) {
              var f = A[b[e]];f && d.push(f);
            }d = c.uniqueArray(d);var g = new RegExp("[\\" + d.join("\\") + "]", "g");return a.replace(g, "");
          }return a;
        } catch (h) {
          throw h;
        }
        ...
        function d(a, b) {
          for (var c = b, d = 0; d < a.length; d++) {
            var e = a[d];e < c.length && c.splice(e, 0, A[e]);
          }return c;
        }var e = a,
        ...
        try {
          var b = k(a),
              c = m(b);return { withDivisors: function withDivisors(a) {
              return a ? c.substr(0, u) : c;
            }, withoutDivisors: function withoutDivisors(a) {
        ...
        var c = [];if (!a) return 0;for (var d = 0; d < a.length; d++) {
          var e = g(d),
              f = a.substr(0, d + 1);if (e && !e.test(f) && (c.push(d), b)) break;
        }return c;
      }function q(a) {
        return p(a, !0)[0];
      }function r(a) {
        for (var b = p(a, !1), c = a, d = 0; d < b.length; d++) {
          var e = b[d],
              f = a.split("");f.splice(e, 1), c = f.join("");
        }return o(c);
      }var s,
          t,
          u = 0,
  ...
  angular.module("ngMask").factory("OptionalService", [function () {
    function a(a) {
      var c = [];try {
        ...
        throw f;
      }return { fromMask: function fromMask() {
          return c;
  ...
  angular.module("ngMask").factory("UtilService", [function () {
    function a(a, b, c) {
      function d(h) {
        var i = a[h],
            j = g[h];if (h === f) for (var k = 0; j > k; ++k) {
          e[h] = i[k], b.apply(c, e);
        } else for (var k = 0; j > k; ++k) {
          e[h] = i[k], d(h + 1);
        }e.pop();
      }c || (c = this);for (var e = [], f = a.length - 1, g = [], h = a.length; h--;) {
        g[h] = a[h].length;
      }d(0);
    }function b(a, b) {
      var c;try {
...
angular.module('timbercrate').service('mainService', function ($http) {
